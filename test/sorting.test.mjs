...
test("网页默认顺序默认从旧到新，也支持从新到旧", () => {
  ...
  assert.deepEqual(
    sortFriends(friends, { criterion: "added" }).map(
      ({ userIdentifier }) => userIdentifier,
    ),
    ["first", "second", "third"],
  );
  assert.deepEqual(
    sortFriends(friends, { criterion: "added", direction: "asc" }).map(
      ({ userIdentifier }) => userIdentifier,
    ),
    ["first", "second", "third"],
  );
  assert.deepEqual(
    sortFriends(friends, { criterion: "added", direction: "desc" }).map(
      ({ userIdentifier }) => userIdentifier,
    ),
    ["third", "second", "first"],
  );
...
test("名称排序支持升序和降序，同名保留传入顺序", () => {
  ...
  assert.deepEqual(
    sortFriends(friends, { criterion: "name", collator }).map(
      ({ userIdentifier }) => userIdentifier,
    ),
    ["b", "a", "z"],
  );
...
test("数值排序的平局保留传入顺序而非网页默认顺序", () => {
  ...
  const cache = refreshCache(createFriendCache(null), [
...
test("上次活跃支持从新到旧和从旧到新，未知活跃时间始终在后", () => {
  ...
  const activities = refreshCache(createFriendCache(null), [
    [
      "older",
      {
        activity: {
...
test("完成条目数按当前范围从高到低或从低到高稳定排序", () => {
  ...
  const values = refreshCache(createFriendCache(null), [
    ["same-b", { completion: { all: 5 }, fetchedAt: 1 }],
    ["high", { completion: { all: 10 }, fetchedAt: 1 }],
